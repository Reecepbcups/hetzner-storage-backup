...
class Backup:
    ...
    def __init__(self, debug=False, showIgnored=True, showSuccess=True):
        ...
        if not "backups" in CONFIG:
            cprint(
                "&cNo backup section in config, grab example from 'secret.json.example'"
            )
            exit(0)
    ...
    def backup_mongodb(self, mongoDBConfig):
        ...
        self.mongodb_abs_location = os.path.join(
            self.backup_path, f"mongodb_dump_{self.current_time}"
        )
        res = os.system(
            f"mongodump --uri={mongoDBConfig['backup-uri']} --out {self.mongodb_abs_location}"
        )
        if res != 0:
        ...
        self.root_paths["mongodb"] = {
            "path": self.mongodb_abs_location,
            "ignore": [],
        }  # We add '/mongodb' here
    ...
    def delete_oldest_files_in_dir_if_over_max(self):
        ...
        if len(list_of_backups) > self.max_local_backups:
            for i in range(len(list_of_backups) - self.max_local_backups):
                full_paths = [
                    f"{os.path.join(self.backup_path, x)}"
                    for x in os.listdir(self.backup_path)
                ]
                ...
                if os.path.isdir(oldest_file):
                    ...
                    cprint(
                        f"&cRemoved {oldest_file} as it was the oldest backup directory"
                    )
                else:
    ...
    def zip_files(self):
        self.zip_file = zipfile.ZipFile(
            self.backup_file_name, "w", compression=zipfile.ZIP_DEFLATED
        )
                ...
                for file in files:
                    ...
                    if not any(re.search(regex, abs_path) for regex in ignore_regex):
                        if self.save_relative:
                            ...
                            )  # saves them as the full abs path
        ...
        if len(self.discord_webhook) > 0:
            ...
            values = {
                ...
                "Storage": [
                    f"{used}/{size} ({storagePercent} used) - Free: {free}",
                    False,
                ],
                "RAM": [
                    f"{usedRam}/{totalRam} ({round(float(percentUsed), 1)}% used)",
                    False,
                ],
            }

            time_passed = (
                datetime.now() - datetime.strptime(self.current_time, TIME_FORMAT)
            ).seconds
        ...
        try:
            ...
            with pysftp.Connection(
                ...
                abs_path = os.path.join(
                    REMOTE_BACKUP_DIR, os.path.basename(self.backup_file_name)
                )
